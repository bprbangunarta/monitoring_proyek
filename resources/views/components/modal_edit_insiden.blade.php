<!-- Modal Laporan -->
<div class="modal fade bd-edit-insiden{{ $insiden->id }}-modal-lg" tabindex="-1" role="dialog"
    aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-dark" id="exampleModalLabel">EDIT INSIDEN</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form method="POST" action="/insiden/{{ $insiden->id }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="modal-body">

                    <div class="input-group mb-3">
                        <input type="text" class="form-control" placeholder="Nama Proyek" name="laporan_id"
                            id="laporan_id" value="{{ $proyek->id }}" hidden>
                    </div>
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="text" class="form-control @error('judul') is-invalid @enderror"
                            placeholder="Nama Pekerjaan" name="judul" id="judul"
                                ...
                                @foreach ($insiden->image as $img)
                                    <div class="card" style="width: 6rem; height: 6rem;">
                                        <div class="image text-center">
                                            <img class="insiden{{ $insiden->id }}{{ $img->id }}-view"
                                                src="/storage/{{ $img->img }}"
                                        ...
                                        <div class="input-group absolute"
                                            ...
                                            <div class="custom-file" style="width: 100%; height: 100%;">
                                                <label
                                                    ...
                                                    <input id="insiden{{ $insiden->id }}{{ $img->id }}"
                                                        name="insiden{{ $insiden->id }}{{ $img->id }}"
                                                        type="file"
                                                        class="pointer-events-none absolute inset-0 h-full w-full opacity-0"
                                                        onchange="viewImg('insiden{{ $insiden->id }}{{ $img->id }}')">
                                                </label>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="input-group mb-3">
                        <textarea class="form-control @error('detail') is-invalid @enderror" rows="3" placeholder="Deskripsi kerja"
                            name="detail" id="detail">{{ old('detail', $insiden->detail) }}</textarea>
                    </div>
